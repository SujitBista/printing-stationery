...
/**
 * Reclassify posted branch-store issues using only reliable receipt evidence.
 *
 * Never deducts Corporate stock, never credits Branch AVAILABLE stock, and
 * never inserts stock-ledger rows. Unknown historical issues become
 * NEEDS_REVIEW and are not receivable.
 */
export async function classifyHistoricalItemIssueDeliveries(): Promise<void> {
  const db = getDb();
