...
import { branchImportConfirmInputSchema } from "@printing-stationery/shared";
import {
  confirmBranchImport,
  previewBranchImport,
} from "../services/branches-import.service.js";
import { AppError } from "../utils/errors.js";
import { BRANCH_IMPORT_MAX_FILE_BYTES } from "../utils/branch-import-xlsx.js";

const upload = multer({
  storage: multer.memoryStorage(),
  limits: {
    fileSize: BRANCH_IMPORT_MAX_FILE_BYTES,
    files: 1,
  },
...
export function branchImportUpload(
  req: Request,
  res: Response,
  next: NextFunction,
): void {
...
export async function previewBranchImportHandler(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
  try {
    ...
    const preview = await previewBranchImport(file.buffer);
...
export async function confirmBranchImportHandler(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
  try {
    const input = parseOrThrow(branchImportConfirmInputSchema.safeParse(req.body));
    const result = await confirmBranchImport(input);
