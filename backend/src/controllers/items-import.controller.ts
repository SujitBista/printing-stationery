...
import { itemImportConfirmInputSchema } from "@printing-stationery/shared";
import {
  confirmItemImport,
  previewItemImport,
} from "../services/items-import.service.js";
import { AppError } from "../utils/errors.js";
import { ITEM_IMPORT_MAX_FILE_BYTES } from "../utils/item-import-xlsx.js";

const upload = multer({
  storage: multer.memoryStorage(),
  limits: {
    fileSize: ITEM_IMPORT_MAX_FILE_BYTES,
    files: 1,
  },
...
export function itemImportUpload(
  req: Request,
  res: Response,
  next: NextFunction,
): void {
...
export async function previewItemImportHandler(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
  try {
    ...
    const preview = await previewItemImport(file.buffer);
...
export async function confirmItemImportHandler(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
  try {
    const input = parseOrThrow(itemImportConfirmInputSchema.safeParse(req.body));
    const result = await confirmItemImport(input);
